...
# Check if amneziawg and iptables are installed
...
# Ask for AmneziaWG port to ensure consistency on both servers
...
if [[ "$LOCATION" == "1" ]]; then
    ...
    # Create AmneziaWG interface with obfuscation parameters
    ...
    awg set awg0 listen-port $AWG_PORT private-key /etc/amnezia/amneziawg/private.key \
        jc 6 jmin 40 jmax 800 s1 60 s2 90 h1 12345678 h2 87654321 h3 13579246 h4 24681357
    ...
    # Clean previous rules and apply forward (preserving ports 22, 80, 10052 exception on Iran server)
    iptables -t nat -F
    ...
    # Create AmneziaWG interface with identical obfuscation parameters
    ip link add dev awg0 type amneziawg
